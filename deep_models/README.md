...
本目录新增的是一套**可执行但尚未在本机运行训练**的深度学习分析协议。当前执行环境未安装 `torch` 与 `h5py`，因此截至本次修订，仅完成 Python 语法编译与静态审查；**没有产生任何新的 3D-CNN、HS3I-Net、空间打乱或 mask-only 实测结果**。后续只有在依赖齐全并实际完成脚本运行后，才能把输出标记为“新执行结果”。
...
```powershell
python -m py_compile deep_models/grouped_hs3i_current_data.py
```

该检查只能证明 Python 语法可编译，不能证明 MAT 方向、GPU 训练、数值收敛或结果性能已经得到运行验证。依赖安装后的首次正式运行应先在独立输出目录完成，并将任何运行时修正与实际结果另行登记。
