...
> **Author-facing draft status; remove before submission.** All development results are newly executed from constructed batches 0–7 in `provenance_study/outputs/development_acquisition_calibration/` and are reproduced by `provenance_study/explore_acquisition_calibration.py`. The one-shot locked confirmation (reserved batches 8–9, opposite acquisition cube) was executed once from committed code `b35b6b6` via `provenance_study/run_acquisition_locked_confirmation.py`; its canonical outputs are in `provenance_study/outputs/acquisition_locked_confirmation/`, and one pre-registration deviation (the fourth gate was not re-run on reserved data) is disclosed in that folder's `NOTES.md`. Constructed batches are deterministic subdivisions of the same 16 archived source images; they are not independent physical lots, farms, years, or instruments.
...
On the 252 reserved seeds (batches 8–9 of the opposite cube), the frozen pipeline achieved 93.3% balanced accuracy (0.047 ECE, 0.232 NLL) versus 85.5% for SNV-LR (0.094 ECE, 0.615 NLL); by direction, cube-1→cube-2 was 91.7% and cube-2→cube-1 was 94.7%. The target-normalization gain over source standardization was +1.9 percentage points (95% cluster-bootstrap [+0.00, +3.76], 96.7% of replicates positive), and calibration reduced ECE from 0.066 to 0.047. Three of the four pre-registered directional gates were evaluated in the one-shot run and all passed: the frozen pipeline exceeded SNV-LR (+7.8 pp, [+0.95, +12.2]), target normalization did not reduce accuracy, and calibration reduced ECE. The fourth gate—covariance features must not help—was a development-only negative control (n=1,012) and was deliberately not re-run on reserved data to avoid additional reserved-patch access; this deviation from the pre-registration is recorded in `provenance_study/outputs/acquisition_locked_confirmation/NOTES.md`. Full metrics and per-seed predictions are in `provenance_study/outputs/acquisition_locked_confirmation/`.
